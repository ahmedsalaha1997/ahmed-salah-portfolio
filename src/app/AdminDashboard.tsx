...
function keepEditableList(value: string) {
  const endsWithNewLine = value.endsWith("\n");
  const lines = value.replace(/\r/g, "").split("\n").map((line) => line.trim());
  return lines.filter((line, index) => line || (endsWithNewLine && index === lines.length - 1));
}
